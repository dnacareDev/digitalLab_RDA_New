...
/*
 * 2014.08.19 ������ : [TODO_Encrypt_ARIA256] ARIA256�� ���� �⺻ �ҽ�
 */
public final class Base64 {
	...
	static {
		...
		for (int i = 0; i <= 25; i++) {
			BASE64_ALPHABET_CHAR[i] = (char) (65 + i);
		}
		for (int i = 26; i <= 51; i++) {
			BASE64_ALPHABET_CHAR[i] = (char) (97 + (i - 26));
		}
		for (int i = 52; i <= 61; i++) {
			BASE64_ALPHABET_CHAR[i] = (char) (48 + (i - 52));
		}
		BASE64_ALPHABET_CHAR[62] = '+';
		BASE64_ALPHABET_CHAR[63] = '/';

		for (int i = 0; i < 255; i++) {
			BASE64_ALPHABET_BYTE[i] = -1;
		}
		for (int i = 90; i >= 65; i--) {
			BASE64_ALPHABET_BYTE[i] = (byte) (i - 65);
		}
		for (int i = 122; i >= 97; i--) {
			BASE64_ALPHABET_BYTE[i] = (byte) ((i - 97) + 26);
		}
		for (int i = 57; i >= 48; i--) {
			BASE64_ALPHABET_BYTE[i] = (byte) ((i - 48) + 52);
		}
