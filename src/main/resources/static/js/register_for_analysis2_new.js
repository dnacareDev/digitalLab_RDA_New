window.onload = function() {
    ...
    function rightSwichFuc2() {
        ...
        // 같은 과제명끼리 체크하지 않으면 오른쪽으로 넘기지 못하게 작동하는 코드
        let same_prj_nm = [];
        tr.forEach(item => {
			let check = item.getElementsByTagName('input')[0];
			if(check.checked) {
				same_prj_nm.push(item.cells[5].innerText);
			}
		});
		for(let i=0 ; i<same_prj_nm.length ; i++) {
			if(same_prj_nm[i] != same_prj_nm[0]) {
				alert("다른 과제명은 동시에 체크할 수 없습니다.");
				return false;
			}
		}
        //
