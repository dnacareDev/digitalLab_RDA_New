window.onload = function() {
    ...
    const { seed_pk_list, planMethodList, plan_code } = planDetailData.plan;
    const headings = document.querySelector('.card_list_text');
    const planCode = document.createElement("li");
    planCode.innerText = `- ${plan_code}`;
    headings.append(planCode)

    const method_list = [...document.querySelectorAll('#method_list_view > tr')];      
    planMethodList.forEach((obj) => {
        const index = method_list.findIndex(el => {
            const pk = el.querySelector('.method_pk').dataset.pk;            
            return pk == obj.method_id
        });
        const comment = obj.method_comment;
        if(method_list[index]?.querySelector('.common_checkBox')) {
            method_list[index].querySelector('.common_checkBox').checked = true;
        }
        
        rightSwichFuc(obj?.method_cycle ?? 0, comment)
    })

    const seed_list = [...document.querySelectorAll('#seed_list_view > tr')];

    seed_pk_list.forEach((id) => {
        const index = seed_list.findIndex(el => (el.id) === ("pk_" + id));
        if (seed_list[index].querySelector('.common_checkBox')) seed_list[index].querySelector('.common_checkBox').checked = true;
        rightSwichFuc2()
    })
    

    const reBtn = document.querySelector('.an_list_box .re_btn');
    const addBtn = document.querySelector('.an_list_box .add_btn');    
    ...
    function rightSwichFuc(method_cycle, comment) {
            ...
            if(check.checked) {
                ...
                <div class="for_box_wrap"> 
                    ...
                    <div class="for_box">
                        ...
                        <span class="for_num">${typeof method_cycle === "number" ? method_cycle : 0}</span>
                ...
                const btns = document.querySelectorAll('.right_list .for_box_wrap button');
                btns.forEach(btn => {
    ...
    function forTextaddFuc() {forNum++; forFuc(forNum)};
